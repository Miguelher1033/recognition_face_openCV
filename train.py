...
def train():
    ...
    confirm = messagebox.askyesno(message='¿Desea iniciar el entrenamiento ahora?', title='Entrenamiento')
    print(confirm)
    if confirm:
        fileTrain = 'train.yml'
        ...
        rootPath = os.getcwd()
        rootPath = rootPath + "/Cascades/haarcascade_frontalface_alt2.xml"
        faceCascade = cv2.CascadeClassifier(rootPath)
        ...
        messagebox.showinfo(message='Entrenamiento realizado correctamente', title='Entrenamiento')
    else:
        messagebox.showinfo(message='Entrenamiento cancelado', title='Entrenamiento')